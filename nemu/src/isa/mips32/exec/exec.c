...
static OpcodeEntry special_table [64] = {
  ...
  /* b001 */ IDEX(jr,jr), IDEX(jalr,jalr), IDEX(movz,movz), IDEX(movn,movn), EX(syscall), EMPTY, EMPTY, EMPTY,
  /* b010 */ IDEX(mfhi,mfhi), IDEX(mthi,mthi), IDEX(mflo,mflo), IDEX(mtlo,mtlo), EMPTY, EMPTY, EMPTY, EMPTY,
...
static OpcodeEntry cop0_table[32] = {
  /* b000 */ IDEX(mfc0,mfc0),EMPTY,EMPTY,EMPTY,
